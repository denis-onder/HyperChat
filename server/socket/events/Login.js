  ...
  if (
    ...
    function verifyLogin() {
      db.collection('credentials').countDocuments(
        { username: username.toLowerCase(), hashedPassword: { $exists: true } },
        function (err, count) {
            ...
            db.collection('credentials').findOne(
              ...
              function (err, user) {
                async function getUserVerification() {
                  var userVerification = await verifyPassword(
                    user.hashedPassword,
                    password
                  );
                ...
                getUserVerification().then((userVerification) => {
                  if (userVerification == 'match') {
                    ...
                    socket.emit('login denied', {
                      loginDeniedReason:
                        'Username already exists/Invalid Password',
                    });
                  }
                });
              }
      ...
      function allowLogin() {
        ...
        if (typeof serverListContents[socket.username] == 'undefined') {
          ...
          serverListContents[socket.username].push({
            ServerName: 'HyperLand',
            Image: './cdn/ServerIcons/HyperLand.png',
            Owner: 'Justsnoopy30',
          });
        }
        ...
        // Emit inital messages
        messageModel
          .find()
          .then((messages) => socket.emit('initial message list', messages))
          .catch((err) => {
            // This error should only occur if no "messages" collection is present.
            // If 0 messages are found, it should still only return an empty array.
            console.error(err);
          });
        ...
        // Usage logging
        console.log(
          `${timestamp} | ${socket.username} joined server: ${socket.server}`
        );
      }
    }
  }
